...
OUTPUT_FILE = 'blackjack_probabilities_106cols_traindata.csv'
...
def generate_rows_data(player_cards_ids, dealer_card_id, bust_results, dealer_win_results):
    """각 시뮬레이션 결과를 포함한 여러 row 생성"""
    ...
    for i in range(n_sims):
        ...
        # Output Columns
        row['player_burst'] = int(bust_results[i])
        row['dealer_wins'] = int(dealer_win_results[i])
...
def run_ml_data_generation():
    ...
    print(f"Starting ML Data Generation...")
    print(f"- Output Columns: 104 Inputs + 2 Outputs")
    print(f"- Simulation: {SIMULATION_COUNT} iter/case")
    print(f"- Saving all simulation results (not just probabilities)")
    ...
    cols_order.extend(['player_burst', 'dealer_wins'])
    ...
    for total in range(8, 21):
        for d_rank in dealer_ranks_loop:
            ...
            # 각 시뮬레이션 결과를 row로 저장
            # -------------------------------------------------
            rows = generate_rows_data(p_cards_ids, d_card_id, bust_results, dealer_win_results)
